...
while cap.isOpened():
    ...
    # Focal length
    F = 615
    # for getting the location and confidence of object detected
    for i in range(detections.shape[2]):

        #used for getting the confidence using index
        confidence = detections[0, 0, i, 2]

        #filtering predictions through confidence
        if confidence > args["confidence"]:
            # detection for class label
    ...
    for i in pos_dict.keys():
        if i in close_objects:
            COLOR= (0, 0, 255)
            sound.play()
            cv2.putText(frame, 'ALERT', (startX, y), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 2)

        else:
            COLOR= (0, 255, 0)
            sound.stop()
            cv2.putText(frame, 'SAFE', (startX, y), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
    ...
    cv2.resizeWindow('Frame', 1365, 763)
